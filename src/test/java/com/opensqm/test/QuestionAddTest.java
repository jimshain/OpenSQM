...
public class QuestionAddTest {
	...
	public void setup() {
		...
		HttpPost httpPost = new HttpPost(
				"http://localhost:8080/OpenSQM-1.0/v1.0/categoryInq");
		...
		try {
			...
			categoryInqRq.getRequestHeader().setRquid(
	...
	public void testValidQuestion() {
		...
		CloseableHttpClient httpclient = HttpClients.createDefault();
		HttpPost httpPost = new HttpPost(
				"http://localhost:8080/OpenSQM-1.0/v1.0/questionAdd");
		CloseableHttpResponse response = null;
		String json = null;
		String request = null;
	...
	public void cleanup() {
		...
		CloseableHttpClient httpclient = HttpClients.createDefault();
		HttpPost httpPost = new HttpPost(
				"http://localhost:8080/OpenSQM-1.0/v1.0/questionDel");
		CloseableHttpResponse response = null;
		...
		try {
			request = gson.toJson(questionDelRq);
			System.out.println("Request = " + request);
			httpPost.setEntity(new StringEntity(request));
			response = httpclient.execute(httpPost);
			if (response.getStatusLine().getStatusCode() == 200) {
